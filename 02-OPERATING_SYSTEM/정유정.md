...
![img_1_jyj.png](img%2Fimg_1_jyj.png)

- TLB(Translation Lookaside Buffer)는 가상 주소를 물리 주소로 빠르게 변환하기 위한 캐시입니다. 
  - 매우 작고 빠른 캐시 (64~1024개 정도)
  - MMU 내부에 탑재되어 있으며 CPU에 포함됨
- 페이지 테이블은 메모리에 있어 접근 속도가 느리기 때문에, CPU는 먼저 MMU 내부의 TLB를 조회합니다.
  TLB에 매핑 정보가 있으면 바로 물리 주소로 접근하고(TLB Hit), 없으면 페이지 테이블에서 찾아온 뒤 TLB에 갱신합니다.
...
- 페이지 테이블은 메모리에 있기 때문에 접근에 시간이 걸리지만, TLB는 CPU 내부의 MMU에 탑재된 고속 캐시라 접근 속도가 훨씬 빠릅니다.
  - TLB 쓰면 주소 변환 오버헤드가 줄어듦
...
- MMU(Memory Management Unit)는 가상 주소를 물리 주소로 변환해주는 CPU 내 하드웨어 장치입니다.
- 주소 변환 외에도 메모리 보호 기능을 수행하며, 잘못된 접근 시 예외를 발생시킵니다.
...
- MMU는 CPU 내부에 위치한 하드웨어로, 가상 메모리를 물리 메모리로 매핑하는 기능을 수행합니다. 
- TLB는 이 MMU 내부에 포함된 주소 변환 캐시로, 주소 매핑 정보를 저장해 빠르게 변환할 수 있게 해줍니다.
...
- 멀티코어 시스템에서는 각 코어가 자체 TLB를 가지고 있기 때문에, 하나의 코어에서 페이지 테이블이 변경되면 다른 코어들의 TLB는 오래된 정보를 가질 수 있습니다. 
- 이를 해결하기 위해 운영체제는 TLB Shootdown이라는 방식으로 동기화를 수행하여, 다른 코어에 인터럽트를 보내 TLB 항목을 비우도록 합니다.
이렇게 하면 모든 코어가 최신 주소 매핑 정보를 기준으로 동작할 수 있습니다.
...
- Context Switching 시에는 현재 TLB에 저장된 가상-물리 주소 매핑 정보가 새 프로세스와 일치하지 않아 오류가 발생할 수 있기 때문에 운영체제는 TLB를 비우는 Flsuh() 작업을 수행합니다.
- 하지만 최신 시스템에서는 ASID(Address Space Identifier)를 통해 TLB 내 매핑 정보를 프로세스별로 구분할 수 있고, TLB는 현재 실행 중인 프로세스의 ASID와 일치하는 항목만 사용하기 때문에 전체를 비우지 않아도 충돌 없이 정확한 주소 변환이 가능합니다.
